...
def welcome(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.GET.get('course') and request.GET.get('course') != "0":
            ...
            course_filter = ' AND pertenece.nivel_id=' + request.GET.get('course')
        ...
        cursor.execute(
            'SELECT nivel.id, concat(nivel.nombre, " ",letra.nombre) as Curso FROM pertenece INNER JOIN usuario ON pertenece.usuario_id = usuario.id INNER JOIN nivel ON pertenece.nivel_id = nivel.id INNER JOIN letra ON pertenece.letra_id = letra.id WHERE usuario.username ="'
        ...
        if request.GET.get('school') and request.GET.get('school') != '0':
            if request.GET.get('course') and request.GET.get('course') != '0':
                cursor.execute(
                    ...
                    'AND b.nivel_id IN (SELECT nivel_id FROM pertenece WHERE usuario_id = (SELECT id FROM usuario WHERE username ="'
                    + request.user.username + '"))' + 'AND a.id_reim="' +
                    request.GET.get('reim') + '"' + 'AND b.nivel_id ="' +
...
#FIN REIM ID = 77, NOMBRE = BUSCANDO EL TESORO PERDIDO
        ...
        SPACEsiembrasxsession_quantity_Dictionary = []
        SPACEsiembrasxsession_quantity_graph = 0
        SPACEcosechasxsession_quantity_Dictionary = []
        SPACEcosechasxsession_quantity_graph = 0
        ...
        if reim_num == "300":
            ...
            SPACEcorrects_quantity_graph = len(SPACEcorrects_quantity) * 50 + 30
            ...
            get_siembrasxsession_SPACE_query = get_siembrasxsession_SPACE(request)
            cursor.execute(get_siembrasxsession_SPACE_query)
            queries.append({
                "name": 'SiembrasxSesion',
                "query": get_siembrasxsession_SPACE_query
            })
            get_siembrasxsession_SPACEResponse = cursor.fetchall()
            for row in get_siembrasxsession_SPACEResponse:
                SPACEsiembrasxsession_quantity_Dictionary.append({
                    'name': row[0],
                    'fecha': row[1],
                    'siembras': row[2]
                })
            SPACEsiembrasxsession_quantity_graph = len(get_siembrasxsession_SPACEResponse) * 40 + 20

            get_cosechasxsession_SPACE_query = get_cosechasxsession_SPACE(request)
            cursor.execute(get_cosechasxsession_SPACE_query)
            queries.append({
                "name": 'CosechasxSesionHuerto',
                "query": get_cosechasxsession_SPACE_query
            })
            get_cosechasxsession_SPACEResponse = cursor.fetchall()
            for row in get_cosechasxsession_SPACEResponse:
                SPACEcosechasxsession_quantity_Dictionary.append({
                    'name': row[0],
                    'fecha': row[1],
                    'cosechas': row[2]
                })
            SPACEcosechasxsession_quantity_graph = len(get_cosechasxsession_SPACEResponse) * 40 + 20
...
#Cantidad de Sesiones
        ...
        return render(
            ...
            "users/welcome.html",
            {
                ...
                'SPACEelementoshuerto_quantity_Dictionary':
                    SPACEelementoshuerto_quantity_Dictionary,
                'SPACEsiembrasxsession_quantity_graph':
                    SPACEsiembrasxsession_quantity_graph,
                'SPACEsiembrasxsession_quantity_Dictionary':
                    SPACEsiembrasxsession_quantity_Dictionary,
                'SPACEcosechasxsession_quantity_graph':
                    SPACEcosechasxsession_quantity_graph,
                'SPACEcosechasxsession_quantity_Dictionary':
                    SPACEcosechasxsession_quantity_Dictionary,
